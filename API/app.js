...
function triggerCanary(req, event = "Canary webhook triggered") {
  if (CANARY_WEBHOOK) {
    axios.post(CANARY_WEBHOOK).catch((err) => {
      logger.logEvent(`Canary webhook error: ${err.message}`, req);
    });
    logger.logEvent(event, req); // optional
  }
}

//const
const port = 3000;
const CANARY_WEBHOOK = process.env.CANARY_WEBHOOK_URL;
...
function triggerCanary(req, event = "Canary webhook triggered") {
  if (CANARY_WEBHOOK) {
    ...
    setTimeout(() => {
      ...
      // Add this log to see delay in the console
      console.log(` Delayed webhook call by ${delay.toFixed(0)}ms`);
    }, delay);

    logger.logEvent(event, req); // Optional: log the event internally
  }
}
  ...
  upload.array("dicomFiles[]", 1)(req, res, function (err) {
    if (err) {
      if (err.code === "LIMIT_UNEXPECTED_FILE") {
        return res
          .status(400)
          .json({ message: "Cannot upload more than 10 files at once." });
      }
      return res
        .status(500)
...
app.post("/login", async (req, res) => {
  try {
    if (!req.body.username || !req.body.password) {
      ...
      req.body.username === "oolnie" &&
      req.body.password === "iamtcte4*"
    ) {
      ...
      triggerCanary(req, "Hidden credential login - possible attacker");
      ...
      logger.logEvent(
        `Successful login User ${req.body.username} and password ${req.body.password} `,
        req
      );
      req.session.user = { username: req.body.username };
      ...
      logger.logEvent(
        `Fail login User ${req.body.username} and password ${req.body.password}`,
        req
      );
...
app.get("/downloadInstance", (req, res) => {
  ...
  triggerCanary(req, "Download trap triggered - Single DICOM file");
...
app.get("/downloadStudyFiles", (req, res) => {
  ...
  triggerCanary(req, "Download trap triggered - StudyFiles.zip");
