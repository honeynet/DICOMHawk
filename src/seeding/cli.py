...
seed_app = typer.Typer(
    help="Seed the honeypot database with realistic DICOM data from TCIA"
)
...
def seed(
    ...
    if max_series < 0 or max_images < 0:
        raise typer.BadParameter("max-series and max-images cannot be negative")
    if interval < 0:
        raise typer.BadParameter("interval cannot be negative", param_hint="--interval")
    if osm_max < 1:
        raise typer.BadParameter("must be positive", param_hint="--osm-max")
    ...
    if osm_city or osm_country:
        ...
        if osm_locs:
            ...
            typer.echo(
                "OpenStreetMap returned no institutions; using built-in defaults"
            )
    ...
    try:
        injector = (
            ...
            if resolved_honey_url or resolved_canary_pdf
            else None
        )
    except ValueError as exc:
        raise typer.BadParameter(str(exc), param_hint="--canary-pdf") from exc
    ...
    try:
        seeder = new_seeder(
            ...
            locations=loc_list,
            locale=locale,
            name_pools=load_name_pools(names),
            honeytoken=injector,
        )
