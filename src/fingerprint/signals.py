...
def evaluate(
    signals: dict, header_user_agent: str | None
    ...
    if browser in ("Chrome", "Safari", "Opera", "WeChat") and isinstance(
        product_sub, str
    ):
    ...
    if (
        isinstance(signals.get("functionBind"), dict)
        and "error" in signals["functionBind"]
    ):
    ...
    if isinstance(keys, list):
        hits = [
            k for k in keys if isinstance(k, str) and k.lower() in _DRIVER_ATTRIBUTES
        ]
        if hits:
    ...
    if (
        browser == "Chrome"
        and engine == "Chromium"
        and android is False
        and plugins == 0
    ):
        fired(
            "plugins_length", "HeadlessChrome", "desktop Chrome reporting zero plugins"
        )
    ...
    if isinstance(webgl, dict):
        if (
            webgl.get("vendor") == "Brian Paul"
            and webgl.get("renderer") == "Mesa OffScreen"
        ):
